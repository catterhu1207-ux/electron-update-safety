...
def main() -> int:
    ...
    start = sub.add_parser("start"); start.add_argument("--executable", required=True, type=Path); start.add_argument("--runs-root", required=True, type=Path); start.add_argument("--arg", action="append", default=[]); start.add_argument("--debug-port",type=int)
    ...
    if args.command in {"check", "stage"}:
        ...
        result = IsolatedRun.start(args.executable, args.runs_root, args.arg, debug_port=args.debug_port)._load()
